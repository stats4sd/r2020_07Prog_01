...
And here are 6 random numbers picked from a binomial distribution with parameters size=10 and prob=0.1. Note that a Binomial distribution is the distribution of successes in a a sequence of independent coin-flip-like experiments
```{r simu2, exercise =TRUE}
rbinom(n=6, size=10, prob=0.1)
```
...
This may not be the most intuitive command ever but once you know that in R, a dataframe is a list of columns, in the same way as a vector c(1,2,3,4,5) is a list of values, then you can see that when we write `i in storms` we are actually saying "i in column1, column2, column3, etc.". Therefore `i in storms` will sequentially make i become each column of the dataframe storms. The function `is.numeric()` will check if the column is of type numeric and return either TRUE or FALSE, hence the output.
...
Let's tweak our last for loop by adding an if/else statement inside. For each column of storm, if the column is "numeric", we will calculate and print the mean of that column. Otherwise, we will print a message saying that we can't calculate the mean:
...
Pretty clearly not a normal distribution! And you should see that every time you press "Run" the distribution will change slightly, because a different simulation is being done, but the general shape will remain very similar.
...
```
Every time we press "Rune", a different simulation is made, and we obtain a different mean value. But they should always be fairly close to 10, the mean of the distribution from which they are simulated.
...
Our function works, but it is not really useful for now, since we can't change the value of n. Let's add an argument to our function and replace 200 by the value of this argument. We'll call this argument n_sample:  

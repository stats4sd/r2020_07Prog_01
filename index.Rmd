...
We can also generate random numbers from theoretical distribution, which is useful to simulate data. In R, there are usually four functions associated with a theoretical distribution. One provides the density of the distribution, one gives cumulative probabilities, one is the quantile function and one generates random numbers from the distribution. The names of these functions are identical, except for the first letter (`d`, `p`, `q` or `r`). The functions to generate random numbers are those that start with an `r`. Here is the list of theoretical distribution functions available in base-R. The random generation functions are on the last column. 
...
This may be a bit overwhelming, so here is a small subset of it:
...
These are probably the most commonly used functions to generate random samples for a lot of people. If you feel you need some refresher on the concept of probability distributions, <a href="http://www.zstatistics.com/videos/#/distributions" target="_blank">here </a> is a pretty good series of videos:


The first argument for all of these functions is `n`, the number of random values to generate. The arguments that follow specify some eventual parameters for the distribution.
...
Here are six random numbers coming from a normal distribution with mean 5 and standard deviation 2: 

```{r simu2, exercise =TRUE}
rnorm(6, 5, 2)
```

And here are five random numbers coming from a uniform distribution between 0 and 10:

```{r simu3, exercise =TRUE}
runif(5, 0, 10)
```

The help menu - that you access with `?`. ie: `?rnorm` - is quite good for this types of functions, as they all work the same way, except for the parameters of the distribution.
